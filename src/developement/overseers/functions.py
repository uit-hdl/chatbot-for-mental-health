...
def complete_prompt_and_get_response(
    prompt_template_path_relative="swift_judge_misc/disclaimers_only",
    f_get_response=f_get_response_gpt35turbo,
    prompt_variables_dict={
        "user_message": "Hello!",
        "chatbot_message": "I know noooothing!",
    },
):
    """Loads the prompt template, fills in the variables using the
    prompt-variable dictionary (keys must match those in the {}) and finally
    generates a response."""
    prompt = load_local_prompt(prompt_template_path_relative)
    prompt_completed = prompt.format(**prompt_variables_dict)
    dump_prompt_to_markdown(
        prompt_completed, "files/prompt-completed/prompt_completed.md"
    )
    return f_get_response(prompt_completed)
...
def load_local_prompt(filepath_relative):
    """Loads prompt with path relative to files/prompt-templates/.

    Example: filepath_relative = 'source_fidelity/v0'"""
    return load_textfile_as_string(
        os.path.join(CWD, f"files/prompt-templates/{filepath_relative}.md")
    )
