...
def evaluate_with_overseers(conversation, harvested_syntax, chatbot_id):
    """Overseer chatbots checks the response generated by the main chatbot. The overseer is based on"""
    available_sources = get_sources_available_to_chatbot(chatbot_id)

    if list_intersection(available_sources, harvested_syntax["citations"]):
...
def generate_overseer_response(
    ...
    backend_conversation = generate_and_add_raw_bot_response(
        backend_conversation, CONFIG["model_id"], CONFIG["deployment_name"]
    )
